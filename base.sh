...
recover_action() {
    ...
    wget -q --no-cache "https://raw.githubusercontent.com/petr-tichy/azure-auto-recover/master/${recover_action}.sh"
...
# simple retry logic with a loop
while true; do
    wget -q --no-cache https://raw.githubusercontent.com/petr-tichy/azure-auto-recover/master/"${distro_test}"
    if [[ $? -eq 0 ]]; then
